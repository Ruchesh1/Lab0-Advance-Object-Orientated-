...
// empty by design
	...
	public static int flipBit(int x) {
		if (x == 0) {
			...
			throw new BadBitException("Not a binary number.");
		}
	}
	...
	public static String alternatingCaps(String s) {
		...
		for (int i = 0; i < len; i++) {
			if (i % 2 == 0) {
				alter = alter + Character.toLowerCase(s.charAt(i));
			} else {
				alter = alter + Character.toUpperCase(s.charAt(i));
			}
		}
		return alter;
	}
